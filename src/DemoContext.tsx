...
import source from './data/generated/controlTowerV3.json';

type Row = Record<string, any>;
export type VendorSubmission = { POLineID: string; VendorID: string; NewMilestoneID: string; VendorExpectedCompletionDate: string; VendorReportedMilestoneStatus: 'Not Started' | 'In Progress' | 'Delayed'; DelayReasonID?: string; Comment?: string };
...
const SESSION_KEY = 'jaipur-living-demo-session-v3';
...
const base: DemoState = { poLines: data.poLines, vendorUpdates: data.vendorUpdates || [] };
...
function reducer(state: DemoState, action: { type: 'submit'; input: VendorSubmission } | { type: 'reset' }): DemoState {
  ...
  const etaVariance = Math.max(0, daysBetween(old.PlannedETA, projectedETA)); const milestoneVariance = Math.max(0, daysAtMilestone - Number(next.TargetDays || 0)); const variance = Math.max(etaVariance, milestoneVariance); const vendorStatus = input.VendorReportedMilestoneStatus === 'Delayed' || etaVariance > 0 ? 'Delayed' : input.VendorReportedMilestoneStatus; const delayed = vendorStatus === 'Delayed' || milestoneVariance > 0; const riskPct = delayed ? (variance >= 8 ? .6 : variance >= 4 ? .35 : .15) : 0;
